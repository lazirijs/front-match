  ...
  <div class="grid gap-8 py-6">
    ...
    <div v-if="channelsFilter.length">
      ...
      <div class="grid gap-4 overflow-y-auto">
        <router-link v-for="(channel, index) in channelsFilter" :key="channel" :to="`/channel/team/${index}`"
...
const search = ref('');
const channelsFilter = computed(() => channels.filter(({ name }) => name.toLowerCase().includes(search.value) ));
</script>