<template>
  <div class="grid gap-20 py-6">
    <h1 class="font-bold text-start">قناة جديدة</h1>
    
    <form @submit.prevent="submitForm" class="grid gap-8">
      <input-app :value="channel.name" @update="channel.name = $event" icon="ic:round-tv" placeholder="اسم القناة" :readonly="loading" />
      <input-app :value="channel.category" @update="channel.category = $event" icon="tabler:category-2" placeholder="فئة القناة" :readonly="loading" />
      <div class="space-y-2 overflow-y-auto">
        <input-app v-for="(item, index) in channel.responsible" :key="index" :value="channel.responsible[index]" @update="channel.responsible[index] = $event" icon="fluent:person-24-filled" placeholder="مسؤولة القناة" :readonly="loading" />
        <h6 class="text-gray-400 text-start first-letter:lowercase px-2"><a @click="channel.responsible.push('')" class="underline cursor-pointer">انقر هنا</a> لإضافة مسؤول آخر </h6>
      </div>
      <div class="space-y-2">
        ...
        <h6 class="text-gray-400 text-start first-letter:lowercase px-2">انقر لتحديد لغات القناة</h6>
    ...
    <div class="space-y-4">
      ...
      <h6 class="w-5/12 m-auto text-gray-400 first-letter:lowercase">
        لا يمكنك تعديل معلومات القناة بعد إضافتها
      </h6>
    </div>
  </div>
</template>
